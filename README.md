...
Compares a gene's regulatory wiring between population-averaged GREmLN ARACNe networks and TCGA tumor-state ARACNe networks. Classifies rewiring as low/moderate/high (Jaccard ≥ 0.6 / 0.3 thresholds), then validates conserved regulators via CASCADE experimental data (LINCS, DepMap, super-enhancers, DoRothEA). Output: tiered regulator list (conserved + CASCADE-validated, conserved without experimental support, tumor-acquired only). Tumor-acquired regulators are additionally annotated against IntOGen's cancer-driver compendium (Martínez-Jiménez et al. 2020) — known drivers are flagged with their role (oncogene/tumor_suppressor/mixed/ambiguous), helping distinguish likely real signal from ARACNe network noise. Within the known drivers, a further tissue-matched subset flags which are specifically established as drivers in the queried `cancer_type` itself (not just known drivers somewhere else) — the strongest candidates. If the reference data isn't available for a given call, driver status is reported as unknown rather than absent.
...
Output: pairwise overlap table, convergent core table with CASCADE validation, known-driver status (IntOGen-sourced, same annotation as `compare_network_contexts`), and — per convergent-core regulator — which of the tested cancer types independently tissue-match it as an IntOGen driver, alongside a PubMed pair-novelty verdict, cancer-type-specific divergent regulator lists, and an optional GREmLN baseline section (each type's tumor-vs-population-average-baseline rewiring). Convergent-core regulators that are also known drivers are the highest-priority cross-cancer candidates; those additionally tissue-matched in the same tested cancer types are corroborated by two independent methods (ARACNe network convergence and IntOGen's mutational driver calling) rather than one. Use this to answer "is gene X's tumor regulatory wiring shared between cancer types?" — a distinct question from `compare_network_contexts`, which asks "how much did each tumor type rewire relative to the population-average baseline?"
