...
![Orchestra architecture. Claude Desktop sends a query to the Orchestra MCP server. The decision layer classifies the gene and routes to one of seven analysis paths. For six paths, the evidence layer executes parallel MCP calls to RegNetAgents and CASCADE. For the novelty\_assessment path, the evidence layer queries NCBI E-utilities (PubMed) directly, bypassing child MCP servers. The explanation layer synthesizes cross-system hits and returns a structured report.\label{fig:architecture}](figure_architecture.png)
...
For literature context, `novelty_assessment(gene="EHF", cancer_context="cervical cancer", gene2="STAT3")` queries PubMed for co-occurrence of EHF and STAT3 in cervical cancer abstracts, returning hit count (3 papers, last 2021), experimental vs. computational split, and a novelty verdict of "novel." When `cancer_context` is supplied to `causal_chain_analysis`, `validate_therapeutic_targets`, or `compare_network_contexts`, pair-level queries are embedded automatically: Orchestra queries PubMed for each of the top five identified regulatory edges concurrently and appends a "Regulatory Pair Novelty" table. For example, `compare_network_contexts(gene="FOXM1", cancer_type="hnsc", cancer_context="head and neck squamous")` surfaces FOXM1→TOP2A as having zero experimental papers despite both genes being individually well-studied — an actionable gap that single-gene queries would not reveal.
