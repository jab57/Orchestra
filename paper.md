...
Once running, any MCP-compatible client can call Orchestra's five composite tools.
...
Five biological validation cases confirm that Orchestra's routing, evidence coordination, and synthesis layers function correctly (Table 1).
...
| `compare_cell_contexts` | MYC | cd4_t_cells, cd8_t_cells, epithelial_cell | Hub regulator + pathway enrichment + DoRothEA TF: conserved (3/3); LINCS/DepMap/SE absent in normal contexts |

Table: Validation results across Orchestra's composite tools (five tools implemented; five validated cases shown).
...
In the cross-cell-type comparison case, `compare_cell_contexts(gene="MYC", cell_types=["cd4_t_cells","cd8_t_cells","epithelial_cell"])` runs 6 parallel MCP calls (2 per cell type) and applies conservation scoring across 7 evidence sources. MYC's PageRank hub status, pathway enrichment, and DoRothEA TF regulon confidence are all conserved (3/3) across immune and epithelial contexts — consistent with MYC's role as a canonical oncogenic transcription factor. LINCS knockdown, DepMap essentiality, and super-enhancer sources are absent in these normal-context queries, which is biologically expected: DepMap essentiality data covers cancer cell lines rather than normal cd4_t or epithelial contexts, and MYC super-enhancer associations in dbSUPER reflect cancer-specific regulatory contexts not present in the GREmLN cell type panel. The heatmap correctly distinguishes network-based conservation (robust across biological contexts) from perturbation-based evidence (context-dependent, requiring cancer cell line data). No errors were returned across all three cell types, confirming graceful handling of large regulatory networks — MYC has 427 downstream targets in epithelial_cell, which required an extended per-call timeout in the comparison path.
...
Orchestra is available at [https://github.com/jab57/Orchestra](https://github.com/jab57/Orchestra) under the MIT license. The repository includes 117 unit tests and 1 integration test covering workflow routing, cross-system synthesis, effector path TF partner selection, gene signature enrichment, cross-cell-type conservation scoring, and graceful degradation when one child server is unavailable, with continuous integration via GitHub Actions. Bug reports and feature requests are tracked via the GitHub issue tracker.
