"""
MI-weight ranking follow-up: does ordering the tumor-acquired candidate set by
ARACNe MI edge weight (rank_tumor_acquired=True), instead of RegNetAgents'
default alphabetical order, change the OncoKB hit rate -- alone, and combined
with the existing >=2 corroboration filter?

Same BRCA/COAD panel, same queries, same statistical pipeline as
experiment_corroboration_tumor_acquired.py -- the only difference is
rank_tumor_acquired=True on each run_analysis() call.

Note: candidates are read from tumor_acquired_cascade_validation's keys (the
genes Orchestra actually scored), not by re-deriving "tumor_state_only[:10]"
locally -- that field is always alphabetical regardless of ranking, since
ranking is applied to Orchestra's internal selection, not to the raw
regulators list returned in the response. Reusing the alphabetical field here
would silently score the wrong candidate set under rank_tumor_acquired=True.
"""
import asyncio
import sys
import io
...
# Identical panel to experiment_corroboration_tumor_acquired.py, for direct comparability.
...
OUT_PATH = Path(__file__).parent.parent / "outputs" / "corroboration_ranked_brca_coad_raw.json"
...
async def run_one(workflow: OrchestraWorkflow, gene: str, cancer_type: str, category: str) -> dict:
    ...
    ranking_method = nc.get("tumor_acquired_ranking_method")
    tumor_acquired = list(validation.keys())
